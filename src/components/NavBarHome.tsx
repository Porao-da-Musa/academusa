  ...
  if (isAuthPage) {
    return (
      <nav className="absolute top-0 left-0 w-full p-6 z-50">
        <Link to="/" className="flex items-center gap-2 text-brand-500 hover:text-brand-400 transition-colors w-fit">
          <div className="bg-brand-500/10 p-2 rounded-lg">
             <Dumbbell className="h-6 w-6" />
          </div>
          <span className="text-xl font-bold tracking-tight text-white">Academusa</span>
        </Link>
      </nav>
    );
  }
            ...
            <Link
              ...
              className={`
                ...
                ${isActive 
                  ...
                  : 'text-slate-800 border-transparent hover:text-black hover:border-black'
                }
              `}
            >
              {section.label}
